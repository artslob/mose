...
class PrimaryBusinessScenario(TestCase):
    def setUp(self) -> None:
        self.customer = Customer.objects.create_user(
            "test_customer", "customer@test.com", "123"
        )
        self.wizard = Wizard.objects.create_user(
            "test_wizard", "wizard@test.com", "123", balance=WIZARD_START_BALANCE
        )
        self.student = Student.objects.create_user(
            "test_student", "student@test.com", "123", teacher=self.wizard
        )
        self.spirit = Spirit.objects.create_user(
            "test_spirit", "spirit@test.com", "123", grade=SpiritGrades.FOLIOT.name
        )
    ...
    def check_available_actions(self, wish, expected_action_names: Iterable[str]):
        ...
        actions = set(
            action.get_action_name() for action in response.context["actions"]
        )
    ...
    def test_artifact_deletion(self):
        ...
        url = reverse(
            "wizuber:delete-artifact",
            kwargs=dict(pk=self.wish.candle_artifacts.first().pk),
        )
        ...
        url = reverse(
            "wizuber:delete-artifact",
            kwargs=dict(pk=self.wish.pentacle_artifacts.first().pk),
        )
        response = self.client.post(url)
        self.assertRedirectsToWishDetail(response)
        self.assertEqual(BaseArtifact.objects.count(), 0)
        ...
        self.assertEqual(BaseArtifact.objects.count(), 1)
        url = reverse(
            "wizuber:delete-artifact", kwargs=dict(pk=self.wish.spirit_artifact.pk)
        )
        response = self.client.post(url)
        self.assertRedirectsToWishDetail(response)
        self.assertEqual(BaseArtifact.objects.count(), 0)
    ...
    def test_business_scenario(self):
        ...
        response = self.client.post(
            self.get_url_for_action("spirit-artifact"), dict(spirit=self.spirit.pk)
        )
        self.assertRedirectsToWishDetail(response)
        self.refresh()
    ...
    def candle_artifact_creation(self):
        response = self.client.post(
            self.get_url_for_action("candle-artifact"),
    ...
    def check_pentacle_artifact_creation(self):
        response = self.client.post(
            self.get_url_for_action("pentacle-artifact"),
